...
9. **`per_marker_batch_r2`, `positive_population_table`, `summarize_positive_population`** — All diagnostic functions are importable directly from `spancy_shift.py`. `positive_population_table()` uses per-sample GMM (not Otsu) and now returns two extra columns per row: `density_ratio` (= `counts[threshold_bin] / counts.max()`, 0=valley/1=peak) and `reliable` (= `density_ratio < 0.3`). `summarize_positive_population(pop_table, min_reliable_frac=0.5, label=...)` aggregates per marker, drops markers where <50% of samples are reliable, prints dropped markers (so the filter is transparent), and returns the kept-marker summary. See SpaNCy-Flow design decision #7 for methodology history.
